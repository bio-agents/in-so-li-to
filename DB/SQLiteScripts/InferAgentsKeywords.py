...
# Function to search the different types of EDAM terms for each agent of OpenEBench
...
def retrieve_keywords(i):
    ...
    # Search all the types of EDAM in the agent
...
def retrieveOEBInformation(publication):
    ...
    return listKeywords, typeAgent, setLanguages, setOs, name, label
...
def create_Agents(c_para, conn_para):
    ...
    # Open the file with the information of all agents from OpenEBench
    ...
    # Store the agent information in this variable
    ...
    for obj in data_json:
        ...
        if not isAgent:
            continue
        listKeywords, typeAgent, setLanguages, setOs, name, label = retrieveOEBInformation(obj)
        print(listId, listKeywords, typeAgent, setLanguages, setOs, name, label)
        # If the agent is found, we can store it in the database
        c.execute(f"""INSERT OR IGNORE INTO Agents
                        values ('{name}', '{label}')""")
        
        # If the agent has Type of software information
        if typeAgent:
            # Insert to the table with all the Programming languages
            c.execute(f"""INSERT  OR IGNORE INTO TypeAgent
                        VALUES ('{typeAgent}')""")
            # Insert the relationships between Agents and Languages
            c.execute(f"""INSERT OR IGNORE INTO AgentsToTypeAgent
                        VALUES ('{typeAgent}', '{label}')""")
        # If the agent has Programming Language information
        if setLanguages:
            ...
            for language in setLanguages:
                ...
                c.execute(f"""INSERT OR IGNORE INTO Languages
                            VALUES ('{language}')""")
                # Insert the relationships between Agents and Languages
                c.execute(f"""INSERT OR IGNORE INTO AgentsToLanguages
                            VALUES ('{language}', '{label}')""")
        # If the agent has Operative System information
        if setOs:
            ...
            for op_sys in setOs:
                ...
                c.execute(f"""INSERT OR IGNORE INTO OperativeSystems
                            VALUES ('{op_sys}')""")
                # Insert the relationships between Agents and OS
                c.execute(f"""INSERT OR IGNORE INTO AgentsToOS
                            VALUES ('{op_sys}', '{label}')""")
        # If the agent has EDAM keywords, store them by type of keyword            
        if listKeywords:
            for typeKeyword, keywords in listKeywords.items():
                insert_keywords(label, keywords, typeKeyword)

        for pmid in listId:
            # Store the relationship between article and the agent that it describes with the information from OpenEBench
            c.execute(f'''INSERT OR IGNORE INTO AgentsToPublications
                    values ("{label}","{pmid}")''')
            
            # Udate MetaCitation with the name of the agents
    ...
    #Sum all the n_citations from all the agents with the same values (in case we have the same co-occurence in the same year for more than one publication from a agent)
    c.execute("""
