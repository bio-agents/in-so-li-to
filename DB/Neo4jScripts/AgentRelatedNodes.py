...
def create_agents_nodes(driver, dict_config):
    with driver.session() as session:
        ...
        session.run("""MATCH (r:TypeAgent) DELETE r""")
        ...
        # Creating Agents nodes
        # name: Name of the agent
        # label: Label in bio.agents
        print("Creating Agent nodes")
        session.run("""
            LOAD CSV WITH HEADERS FROM "file:///%s" AS csv
            CREATE (p:Agent {name: csv.name, label: csv.label})
            """ % (dict_config["agent_nodes"]))
        
        # Creating Type of agent nodes
        # name: Name of the type of agent
        print("Creating Type of agent nodes")
        session.run("""
            LOAD CSV WITH HEADERS FROM "file:///%s" AS csv
            CREATE (p:TypeAgent {name: csv.Type})
            """ % (dict_config["type_nodes"]))
        
        # Creating Agent-TypeAgent edges
        print("Creating HAS_TYPE edges")
        session.run("""
            LOAD CSV WITH HEADERS FROM "file:///%s" AS csv
            MATCH (t:Agent {label:csv.label}),(k:TypeAgent {name:csv.Type})
            CREATE (t)-[:HAS_TYPE]->(k)
            """ % (dict_config["agent_type_edges"]))
        ...
        # Creating Agent-Language edges
        print("Creating USE_LANGUAGE edges")
        session.run("""
            LOAD CSV WITH HEADERS FROM "file:///%s" AS csv
            MATCH (t:Agent {label:csv.label}),(k:Language {name:csv.Language})
            CREATE (t)-[:USE_LANGUAGE]->(k)
            """ % (dict_config["agent_language_edges"]))
        ...
        # Creating Agent-OS edges
        print("Creating USE_OS edges")
        session.run("""
            LOAD CSV WITH HEADERS FROM "file:///%s" AS csv
            MATCH (t:Agent {label:csv.label}),(k:OS {name:csv.os})
            CREATE (t)-[:USE_OS]->(k)
            """ % (dict_config["agent_os_edges"]))
        ...
        for i in range(len(list_edam_relationships)):
            ...
            session.run("""
                LOAD CSV WITH HEADERS FROM "file:///%s" AS csv
                MATCH (t:Agent {label:csv.label}),(k:Keyword {edam:csv.keyword})
                CREATE (t)-[:%s]->(k)
        ...
        # Creating Agent-Publications edges
        # :HAS_TOOL: Label of the edges
        print("Creating Agent-Publication edges")
        session.run("""
            LOAD CSV WITH HEADERS FROM "file:///%s" AS csv
            MATCH (t:Agent {label:csv.label}),(p:Publication {id:csv.Publication_id})
            CREATE (p)-[:HAS_TOOL]->(t)
            """ % (dict_config["agent_publication_nodes"]))
