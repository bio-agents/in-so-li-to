...
import AgentTopicData from '../../DB/AgentTopicAutocomplete.json';
...
function updateNodes(){
	// Take name and id of all the agents and topics in the Label Menu
	// Store the values in the dictionary
	var nameNodeDict = {};
	['AgentButton','topicDiv'].forEach( className => {
...
$(function () {
	$('#agenttopic_autocomplete').autocomplete({
		source: function(request, response) {
			...
			function subarray(matcher) {
				return $.grep(AgentTopicData, function(item) {
		...
		.autocomplete('instance')._renderItem = function (ul, item) {
			if (item.labelnode[0] === 'Agent'){
				return $('<li><div class="boxAutocomplete"><img src="' + AgentImage +'"><div><div class="TextAutocomplete">' + item.value + '</div><div class="typeSoft">' + item.type.join('/') + '</div></div></div></li>').appendTo(ul);
			}
...
function addLegend() {
	...
	if(optionRadio.value==='Normal'){
		// Insert the different type of nodes in the legend (Publication, Agent, Dataset)

		list.innerHTML = '<div id="legendnormal"><span id="ExpandedNode" style="background-color:#fbba7e;"></span><span> Expanded node </span></div>';
		list.innerHTML += '<div id="legendnormal"><img style="background-color: #add8e6;" src=' + AgentImage + ' ><span> Agents </span></div>';
...
// And store the color representing the type of node that they are (Publication, Agent, Database)
function storeClusterColor(){
	setTimeout(function() {
		var net = Vis.body;
		var allNodes = net.nodeIndices;

		var listLegend = document.getElementsByClassName('AgentButton');
		var centeredNodes = [];
		for (var i = 0; i < listLegend.length; i++) {
		...
		allNodes.forEach((node) => {
			...
			if (net.nodes[node].options.Neo4jLabel==='Agent'){
...
// Remove all the Agents and Topics from the Label Menu
function removeAllAgentsMenu() {
...
async function addNodesGraph(nameNode, idNode, nodeType) {
	...
	if(nodeType==='Topic'){
		...
		addAgentLabelMenu(nameNode, idNode);
	}
...
function addNodes(nameNode, idNode, nodeType) {
	...
	Array.prototype.forEach.call(list, function (agent) {
		if (agent.textContent === nameNode) {
			isInMenu = true;
		}
	});
	// If name of agent/topic not in menu
	if (isInMenu === false) {
...
// Add agents and topics displayed in the webpage in the Label Menu
// Also, when they are click, remove their nodes from the graph
function addAgentLabelMenu(NameTopic, idNode) {

	var buttonAgent = document.createElement('button');
	buttonAgent.className= 'AgentButton';
	// buttonAgent.innerText = NameTopic;
	buttonAgent.value = idNode;

	buttonAgent.innerHTML='<img class="close-icon" src="' + CloseButton + '"/>' +
	'<div class="name-topic">' + NameTopic + '</div>';

	document.getElementById('agents-list').appendChild(buttonAgent);

	var buttonAgent=document.getElementsByClassName('AgentButton');
	for (var i = 0; i < buttonAgent.length; i++){
		buttonAgent[i].addEventListener('click', function (e) {
			// Store node ID
			var IdAgent = e.currentTarget.value;
			e.currentTarget.parentNode.removeChild(e.currentTarget);
			// Take all the nodes connected to the agent clicked
			var ConnectedNodes = Vis.getConnectedNodes(IdAgent);
			...
			// Remove the agent selected and the nodes connected to it having 1 connection
			Vis.selectNodes([IdAgent].concat(UnconnectedNodes));
			Vis.deleteSelected();
...
function menu(e1) {
	...
	if (e1.nodes.length === 1) {
		...
		contextMenu.innerHTML = '<div class="item" id="nameAgent">' + name + '</div><div class="topicmenu" id="topic"></div><div class="item" id = "webpage"></div><div class="item" id="center"></div><div class="item" id="expand"></div>'
